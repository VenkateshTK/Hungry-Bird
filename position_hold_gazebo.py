...
class Edrone():
	...
	def pitch_set_pid(self,alt):
		self.kp[1] = alt.Kp * 0.01 # This is just for an example. You can change the ratio/fraction value accordingly
		self.ki[1] = alt.Ki * 0.008
		self.kd[1] = alt.Kd * 0.3
 
	def roll_set_pid(self,alt):
		self.kp[0] = alt.Kp * 0.01 # This is just for an example. You can change the ratio/fraction value accordingly
	...
	def pid(self):
		 ...
		 self.error_sum[0] = self.prev_error[0]+self.error[0]
		 self.error_sum[1] = self.prev_error[1]+self.error[1]
		 self.error_sum[2] = self.prev_error[2]+self.error[2] 


		 self.out_roll=self.kp[0]*self.error[0]+self.kd[0]*(self.prev_error[0]-self.error[0])+self.ki[0]*(self.error_sum[0])
		 self.out_pitch=self.kp[1]*self.error[1]+self.kd[1]*(self.prev_error[1]-self.error[1])+self.ki[1]*(self.error_sum[1])
		 self.out_altitude=self.kp[2]*self.error[2]+self.kd[2]*(self.prev_error[2]-self.error[2])+self.ki[2]*(self.error_sum[2])
