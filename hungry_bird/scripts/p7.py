	...
	#	8. Add error_sum
		...
		if self.mode == 0:
			for i in range(0,3):
		 	...
		 	for i in range(0,3):
				self.out[i]= (1-math.exp(-math.fabs(self.error[i])))*self.Kp[i]*self.error[i]+(1-math.exp(-math.fabs(self.error[i])))*self.Kd[i]*(self.error[i]-self.last_error[i])+self.Ki[i]*self.error_sum[i]
