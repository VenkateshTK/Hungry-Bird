...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint = [5.68,-1.91,33.40] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		self.f=1
		...
		self.data_vrep=	rospy.Publisher('/message_vrep', String, queue_size=10)
	...
	#	8. Add error_sum
		...
		if ((self.error[0]>-0.5) and (self.error[0] <0.5)) :
			self.error_sum[0]=0	
		if ((self.error[1]>-0.5) and (self.error[1] <0.5)) :
			self.error_sum[1]=0
		if ((self.error[2]>-0.5) and (self.error[2] <0.5) and (self.error[0]>-0.5) and (self.error[0] <0.5) and (self.error[1]>-0.5) and (self.error[1] <0.5) and (self.f==1)):
			if (self.f==1):
				self.data_vrep.publish('1')
			self.waypoints	
			self.j+=1		
	#------------------------------------------------------------------------------------------------------------------------
		#rospy.sleep(self.sample_time)
