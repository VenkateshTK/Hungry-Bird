...
class Edrone():
	...
	def __init__(self):
		...
		self.initial_setpoint = [0,0,20,0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		...
		rospy.Subscriber('/yawdata', Float64, self.yaw_callback)
	...
	#-----------------------------Write the PID algorithm here--------------------------------------------------------------
		...
		for i in range(0,3):
			self.out[i]= self.Kp[i]*self.error[i]+self.Kd[i]*(self.error[i]-self.last_error[i])/self.sample_time+self.Ki[i]*self.error_sum[i]*self.sample_time
		...
		if (self.error[0] <1) and (self.error[0] >-1):
		 	self.error_sum[2]=0	
		if (self.error[1] <1) and (self.error[1] >-1):
			self.error_sum[1]=0	
		if (self.error[2] <1) and (self.error[2] >-1):
			self.error_sum[0]=0		
