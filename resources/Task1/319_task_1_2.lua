function sysCall_init()
    ...
    start_handle = sim.getObjectHandle('initial_waypoint')
...
function sysCall_actuation()
    ...
    if compute_path_flag == true then
        if mode == '1' then
            ...
            goal_pose = getpose(goal1_handle,-1)
            -- Setting start state
        ...
        if mode == '2' then
            -- Getting startpose
            start_pose = getpose(goal1_handle,-1)
            -- Getting the goalpose
            goal_pose = getpose(goal2_handle,-1)
            -- Setting start state
            simOMPL.setStartState(t,start_pose)
            -- Setting goal state but the orientation is set same as that of startpose
            simOMPL.setGoalState(t,{goal_pose[1],goal_pose[2],goal_pose[3],start_pose[4],start_pose[5],start_pose[6],start_pose[7]})
        ...
        if mode == '3' then
            -- Getting startpose
            start_pose = getpose(goal2_handle,-1)
            -- Getting the goalpose
